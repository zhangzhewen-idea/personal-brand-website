...
public class MatrixAccountGatewayImpl extends AbstractManagedGateway<MatrixAccount, MatrixAccountPO> implements MatrixAccountGateway {
    ...
    @Override public List<MatrixAccount> listActive() { return mapper.selectList(new QueryWrapper<MatrixAccountPO>().eq("`is_deleted`", false).orderByAsc("`create_time`", "`id`")).stream().map(this::toDomain).toList(); }
    @Override protected MatrixAccount toDomain(MatrixAccountPO po) { return new MatrixAccount(PersistenceSupport.toBase(po), po.getPlatformName(), po.getPlatformLogo(), po.getAccountUrl(), po.getIntro(), po.getFollowerCountText(), po.getColorClass()); }
    @Override protected MatrixAccountPO toPO(MatrixAccount m) { MatrixAccountPO po = new MatrixAccountPO(); PersistenceSupport.copyBase(m.base(), po); po.setPlatformName(m.platformName()); po.setPlatformLogo(m.platformLogo()); po.setAccountUrl(m.accountUrl()); po.setIntro(m.intro()); po.setFollowerCountText(m.followerCountText()); po.setColorClass(m.colorClass()); return po; }
}
